...
// 本次 session 中已確認失效的代理器（例如需要付費 API Key），直接跳過以避免每檔標的都重試一次
const deadProxyIds = new Set();
...
export async function fetchYahooData(symbol, options = {}) {
    ...
    const endpoint = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=6mo&_=${cacheKey}`;
    ...
    for (let attempt = 0; attempt < PROXY_GENERATORS.length; attempt++) {
        ...
        // 已知失效（例如需付費 Key）的代理器直接跳過，避免每檔標的都重試一次
        if (deadProxyIds.has(proxy.id)) continue;
        ...
        try {
            ...
            if (response.status === 401 || response.status === 403) {
                deadProxyIds.add(proxy.id); // 永久失效（需付費或被封鎖），本次 session 不再嘗試
            ...
            // 若已無其他代理可試（皆失效或已跑完），不再等待，直接結束
            if (deadProxyIds.size >= PROXY_GENERATORS.length) break;
...
export async function fetchStockData(item, options = {}) {
    ...
    try {
        ...
        return buildAnalysisFromYahooData(item, parsed, fetchedAt);
    } catch (error) {
